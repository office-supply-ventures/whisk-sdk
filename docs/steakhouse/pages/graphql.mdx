...
The [Whisk GraphQL playground](https://api-v2.whisk.so/graphql) is the best way to explore the schema and test queries before writing code. This is the production API — use your production API key here.

:::info
The Whisk API is currently read-only. Mutations are not supported.
:::
...
To write custom queries, install `@whisk/graphql` alongside `@steakhouse/sdk`:
